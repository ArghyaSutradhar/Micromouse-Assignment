...
pygame.display.set_caption("Path Finding With Micromouse")
screen.fill(white)
...
class spot:
    ...
    def addNeighbors(self, grid):
        i = self.i
        j = self.j
        if i < cols-1 and grid[self.i + 1][j].obs == False: # Down
            self.neighbors.append(grid[self.i + 1][j])

        if i > 0 and grid[self.i - 1][j].obs == False: # Up
            self.neighbors.append(grid[self.i - 1][j])

        if j < row-1 and grid[self.i][j + 1].obs == False: # Right
            self.neighbors.append(grid[self.i][j + 1])

        if j > 0 and grid[self.i][j - 1].obs == False: # Left
            self.neighbors.append(grid[self.i][j - 1])
...
def main():
    ...
    if len(openSet) > 0:
        ...
        if current == end:
            ...
            Tk().wm_withdraw()
            result = messagebox.askokcancel('Program Finished', ('Would you like to re-run the program?'))
            if result == True:
